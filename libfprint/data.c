...
static void storage_setup(char *fp_path)
{
	base_store = g_build_filename(fp_path, ".fprint", NULL);
...
{
	return print_data_new(dev->drv->id, dev->devtype, fpi_driver_get_data_type(dev->drv));
}
...
API_EXPORTED size_t fp_print_data_get_data(struct fp_print_data *data,
	unsigned char **ret)
{
	struct fpi_print_data_fp1 *out_data;
	struct fpi_print_data_item_fp1 *out_item;
	...
	*ret = (unsigned char *) out_data;
	...
	while (list_item) {
		item = list_item->data;
		out_item = (struct fpi_print_data_item_fp1 *)buf;
	...
	size_t buflen)
{
	...
	struct fpi_print_data_item_fp1 *raw_item;
	...
	while (total_data_len) {
		...
		raw_item = (struct fpi_print_data_item_fp1 *)raw_buf;
...
static char *get_path_to_storedir(uint16_t driver_id)
{
	char idstr[5];

	g_snprintf(idstr, sizeof(idstr), "%04x", driver_id);

	return g_build_filename(base_store, idstr, NULL);
}

static char *__get_path_to_print(uint16_t driver_id, enum fp_finger finger)
{
	char *dirpath;
	char *path;
	char fingername[2];
	...
	dirpath = get_path_to_storedir(driver_id);
...
{
	return __get_path_to_print(dev->drv->id, finger);
}
...
API_EXPORTED int fp_print_data_save(struct fp_print_data *data,
	enum fp_finger finger, char *fp_path)
{
	GError *err = NULL;
	char *path;
	char *dirpath;
	...
	path = __get_path_to_print(data->driver_id, finger);
...
API_EXPORTED int fp_print_data_load(struct fp_dev *dev,
	enum fp_finger finger, struct fp_print_data **data, char *fp_path)
{
	gchar *path;
	struct fp_print_data *fdata;
	int r;
...
{
	if (data){
//	    g_slist_foreach (data->prints, (GFunc) fpi_print_data_item_free, NULL);
	    g_slist_foreach (data->prints, (GFunc) g_free, NULL);
	    g_slist_free(data->prints);
	}
	g_free(data);
}
 ...
 */
static gint sorter( gconstpointer a, gconstpointer b )
{
        struct fp_dscv_print *printa;
        struct fp_dscv_print *printb;

        printa = a;
        printb = b;
        return( strcmp( printa->path, printb->path ) );
}
...
API_EXPORTED struct fp_dscv_print **fp_discover_prints(char *fp_path)
{
	...
	while ((ent = g_dir_read_name(dir))) {
		...
		tmplist = scan_dev_store_dir(path, driver_id, 1, tmplist);
		g_free(path);
	}
	...
	for (i = 0; i < tmplist_len; i++, elem = g_slist_next(elem)){
		list[i] = elem->data;
        	fp_dbg("discovered figner: %d at %s\n", list[i]->finger, list[i]->path);
	}
