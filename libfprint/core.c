	...
	const char *function, const char *format, ...)
{
	...
	ALOGD("%s:%s [%s] %s", component ? component : "fp", prefix, function, buff);
...
static struct fp_driver *find_supporting_driver()
{
	GSList *elem = registered_drivers;
	
	struct fp_driver *drv = elem->data;
	fp_dbg("driver %s",drv->name);
	return drv;
}

static struct fp_dscv_dev *discover_dev()
{

	struct fp_driver *drv = find_supporting_driver();
...
API_EXPORTED int fp_dev_mode(struct fp_dev *dev, int sensor_mode)
{
	...
	if (!imgdrv->mode) {
		fp_err("img driver %s has no mode func", drv->name);
		return -ENOTSUP;
	}

	return imgdrv->mode(imgdev, sensor_mode);
}
	...
	struct fp_print_data **print_data, struct fp_img **img)
{
	...
	if (stage == -1) {
		...
		imgdev->action = IMG_ACTION_ENROLL;
	...
	struct fp_img **img)
{
	...
	case FP_VERIFY_NO_MATCH:
		...
		break;
	case FP_VERIFY_MATCH:
		fp_dbg("result: match at offset %d", *match_offset);
		break;
	case FP_TIMEOUT:
	case FP_VERIFY_RETRY:
		fp_dbg("verify should retry");
